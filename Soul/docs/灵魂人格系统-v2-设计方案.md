...
4. 任务执行受情绪门控，采用“阈值二元门控”：触发锁定时阻断，否则必执行。
...
- 低情绪输入衰减（当前实现）：
  - 先计算情绪能量 `E = 0.55*intensity + 0.30*||v_user|| + 0.15*max(negative, positive)`。
  - 当输入为 `neutral/calm` 且 `E < 0.42` 时，冲击缩放为 `0.03`（几乎不影响 PAD）。
  - 其余低能量分段缩放：`E<0.22 -> 0.03`，`E<0.35 -> 0.08`，`E<0.50 -> 0.18`，否则 `1.0`。
...
即：除非触发阈值并进入锁定，否则任务必然执行，不再做连续概率衰减。
...
- 状态链路：`p/a/d`, `b/s/m`, `exec_probability(0|1)`, `exec_mode`
