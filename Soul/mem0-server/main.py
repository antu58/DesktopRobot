...
if MEM0_EMBED_PROVIDER == "fastembed":
    # fastembed returns ndarray in some versions; pgvector expects plain Python lists.
    from mem0.embeddings.fastembed import FastEmbedEmbedding

    _fastembed_embed = FastEmbedEmbedding.embed

    def _embed_with_list(self, text, memory_action=None):
        vector = _fastembed_embed(self, text, memory_action)
        if hasattr(vector, "tolist"):
            return vector.tolist()
        return vector

    FastEmbedEmbedding.embed = _embed_with_list
...
DEFAULT_CONFIG = {
    ...
    "embedder": {"provider": MEM0_EMBED_PROVIDER, "config": embedder_config},
